...
func (mysql *MYSQL) LogIn(userLog *entities.UserLogIn) (*entities.User, error) {
	...
	var idJefeNull sql.NullInt32 // Usar NullInt32 para manejar valores NULL

	err := mysql.db.QueryRow("SELECT id, name, password, rol, email, username, id_jefe FROM users WHERE username = ?",
		userLog.Username).Scan(&user.ID, &user.Name, &user.Password, &user.Rol, &user.Email, &user.Username, &idJefeNull)

	if err != nil {
		return nil, err
	}

	// Convertir sql.NullInt32 a *int
	if idJefeNull.Valid {
		intValue := int(idJefeNull.Int32)
		user.Id_jefe = &intValue
	} else {
		user.Id_jefe = nil // Explícitamente establece nil si el valor es NULL
	}
...
func (mysql *MYSQL) GetAll() ([]entities.UserResponse, error) {
	...
	for rows.Next() {
		...
		var idJefeNull sql.NullInt32

		if err := rows.Scan(&user.ID, &user.Name, &user.Rol, &user.Email, &user.Username, &idJefeNull); err != nil {
			return nil, err
		}
...
func (mysql *MYSQL) GetByID(id int32) (*entities.UserResponse, error) {
	...
	err := mysql.db.QueryRow("SELECT id, name, rol, email, username, id_jefe FROM users WHERE id = ?", id).
		Scan(&user.ID, &user.Name, &user.Rol, &user.Email, &user.Username, &idJefeNull)
